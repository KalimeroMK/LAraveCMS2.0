...
class HomeController extends BaseController
{
    ...
    {
        $featured_ids = Posts::where('featured', 1)->where('render_type', '!=', Posts::RENDER_TYPE_TEXT)->orderBy('created_at', 'desc')->where('status', Posts::STATUS_PUBLISHED)->limit(6)->pluck('id');
        ...
        $this->data['latest_top'] = Posts::orderBy('created_at', 'desc')->where('render_type', '!=', Posts::RENDER_TYPE_TEXT)->orderBy('views', 'desc')->whereNotIn('id', array_merge($featured_ids->toArray(), $this->data['just_posted']->pluck('id')->toArray()))->where('status', Posts::STATUS_PUBLISHED)->limit(5)->get();
        ...
        foreach ($categories as $category) {
            $sub_categories_ids = SubCategories::where('parent_id', $category->id)->pluck('id');

            $posts = Posts::orderBy('created_at', 'desc')->where('render_type', '!=', Posts::RENDER_TYPE_TEXT)->whereNotIn('id', array_merge(array_merge($featured_ids->toArray(), $this->data['just_posted']->pluck('id')->toArray()), $this->data['latest_top']->pluck('id')->toArray()))->whereIn('category_id', $sub_categories_ids)->where('status', Posts::STATUS_PUBLISHED)->limit(7)->get();
    ...
    public function byCategory($category_slug)
    {
        ...
        $sub_cat_ids = SubCategories::where('parent_id', $category->id)->pluck('id');
    ...
    public function byTag($tag_slug)
    {
        ...
        $post_ids = PostTags::where('tag_id', $tag->id)->pluck('post_id');
    ...
    public function article($slug)
    {
        ...
        if ($post->rating_box == 1) {
            $all_ratings = PostRatings::orderBy('created_at', 'desc')->where('post_id', $post->id)->where('approved', 1)->pluck('rating');
        ...
        if (sizeof($tag_ids->pluck('tag_id')) > 0)
            $post->tags = Tags::whereIn('id', $tag_ids->pluck('tag_id'))->get();
        else
            $post->tags = [];
    ...
    public function categoryRss($slug)
    {
        ...
        $sub_ids = SubCategories::where('parent_id', $category->id)->pluck('id');
